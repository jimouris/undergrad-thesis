...
\begin{thesisprologue}[Preface]
    This project was developed in Athens, Greece between September 2015 and April 2016. In the very beginning of this work, I had to understand how the Doop framework works for pointer or points-to analysis. I also had to get used to Soot framework, which intergrates with Doop to translate Java bytecode to a three address code language (Jimple IR) and produce the facts for the analysis. After all these, I tried to understand the way Java bytecode to Jimple translation is implemented and tried to parallelize it. 
\end{thesisprologue}
...
\thesissection{Doop}
    ...
    \thesissubsection{Fact Generation}
        ...
        The main problem is that fact generation takes more than 50\% of total execution time (either jimple or shimple). Below are presented a few time examples of the sequential Doop Fact Generation and total execution time (both fact generation and analysis times) with Soot 2.5.0 version.
    ...
    \thesissubsection{Fact Table Example}
    	After the fact generation finishes, Doop executes queries to the database and computes the results depending on the analysis. At the end of an analysis, a symbolic link is created for the resulting database, and another one at top level called last-analysis, each time pointing to the latest successful analysis. \\
        \\
...
\end{lstlisting}
    ...
    \thesissubsection{Soot Side}
	    The way Soot is implemented, it has a class that encloses all global objects (\texttt{G.java}). As we mentioned before, Soot does much more than just translating bytecode to jimple, so it has various phases and a lot of different options for transformations given. The phase that translates bytecode to jimple is jb phase. In this phase we found all global objects, the methods that affects them and the methods that those global objects call and we synchronized them and/or made the thread-local. 
