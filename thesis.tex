...
\begin{thesisabstract}[ABSTRACT]
	In recent years become increasingly popular to use inductive databases to declarative program analysis. One of the most significant cases are databases using Datalog. The production of the initial information (EDB) that is inserted to the database for the analyzed program is in many cases also costly in time by analyzing the program, especially as the size thereof increases.\\
	\\
...
\thesissection{Doop}
    ...
    The advantage of Doop compared to alternative context-sensitive pointer analysis implementations (such as Paddle), is that Doop is much faster, and scales better. Also, with comparable context-sensitivity features, Doop is more precise in handling some Java features (for example exceptions) than alternatives. //kati prepei na alla3ei edw..
    ...
    \thesissubsection{Fact Table Example}
        ...
        Note: Explain what each of the below EDB relations is (describe the arguments of each predicate, check doop-nexgen/logic/facts/declarations.logic).
    	\begin{figure}[H]
\begin{lstlisting}
