...
\thesissection{Doop}
    ...
    \thesissubsection{Fact Table Example}
    	After the fact generation finishes, Doop executes queries to the database and computes the results depending on the analysis. At the end of an analysis, a symbolic link for the results is created for the resulting database. Also, for convenience, a second symbolic link is created at top level called last-analysis, each time pointing to the latest successful analysis.\\
        \\
...
\thesissection{Soot}
    ...
    \thesissubsection{Applying the latest Soot version}
    	Doop used Soot version 2.5.0, to minimize the time consumed in fact generation phase, before we tried to parallelize it, we applied the latest (Sept. 2015 develop branch) Soot version. We faced a lot of compatibility problems in order to use it properly but it gave a speed up of 40\%. Below are presented a few time examples with Soot-2.5.0 and the latest one.
